<?php
$attribute = $this->getAttribute();
// echo'<pre>';
echo "HEY..!! WELL DONE..!!  :)";
?>
<div class="container-fluid">
 
<a class="btn btn-success mt-4 float-right" onclick="object.setUrl('<?php echo $this->geturl('form',null,null,true);?>').resetParams().load()">Create Attribute <i class="fa fa-plus" aria-hidden="true"></i></a><br><br><br><br>
 <div class="card">
<div id="table">
    <table class="table table-hover">
      <thead class="thead-light">
        <tr>
            <th>Attribute Id</th>
            <th>Entity Type Id</th>
            <th>Name </th>
            <th>Code</th>
            <th>Inpute Type</th>
            <th>BackendType</th>
            <th>Sort Order</th>
            <th>Backend Model</th>
            <th>Action</th>
        </tr>
      </thead>
      <tbody>
        <?php if(!$attribute): ?>
        <tr>
          <td>No Record Found..!! :(</td>
        </tr>

        <?php else: ?>
        <?php foreach($attribute->getData() as $key=>$value): ?>
            <tr>
              <td><?php echo $value->attributeId; ?></td>
              <td><?php echo $value->entityTypeId; ?></td>
              <td><?php echo $value->name; ?></td>
              <td><?php echo $value->code; ?></td>
              <td><?php echo $value->inputType; ?></td>
              <td><?php echo $value->backendType; ?></td>
              <td><?php echo $value->sortOrder; ?></td>
              <td><?php echo $value->backendModel; ?></td>
              <td><a onclick="object.setUrl('<?php echo $this->geturl('form','attribute',['attributeId' => $value->attributeId]);?>').resetParams().load()" title="Update Contact"  class=" btn btn-warning" role="button">Update</a>
              <a onclick="object.setUrl('<?php echo $this->geturl('delete','attribute',['attributeId'=>$value->attributeId]);?>').resetParams().load()" title="Delete Product" class=" btn btn-danger " role="button">Delete</a>
            </td>
            </tr>
          <?php endforeach; ?>
        <?php endif; ?>
      </tbody>
    </table>
</div>
</div>
</div>
